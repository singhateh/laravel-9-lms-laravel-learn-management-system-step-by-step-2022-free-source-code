<div id="accordion">
    ...
    <div class="card card-primary">
        <div class="card-header d-flex p-0">
            <h4 class="card-title p-3">
                <a data-toggle="collapse" aria-expanded="true" data-parent="#accordion"
                    href="#{{ isset($title) ? Str::slug($title) : 'permissionHeading' }}">
        ...
        <div id="{{ isset($title) ? Str::slug($title) : 'permissionHeading' }}"
            class="panel-collapse collapse {{ $keys == 0 ? 'show' : '' }}">
                        ...
                        <div class="col-md-3">
                            <div class="checkbox">
                                <label class="{{ Str::contains($perm->name, 'delete') ? 'text-danger' : '' }}">
                                    {!! Form::checkbox('permissions[]', $perm->name, $per_found, isset($options) ? $options : []) !!} {{ Str::replace('_', ' ', Str::headline($perm->name)) }}
                                </label>
                            </div>
                        </div>
                    @endforeach
